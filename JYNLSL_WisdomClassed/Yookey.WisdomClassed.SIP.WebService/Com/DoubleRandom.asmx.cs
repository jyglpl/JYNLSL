...
namespace Yookey.WisdomClassed.SIP.WebService.DoubleRandom
{
    ...
    public class DoubleRandom : System.Web.Services.WebService
    {
        ...
        public string GetDoubleRandomList(string context)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(context))
                {
                    ...
                    if (typeId == "0")
                    {
                        //List<DoubleRandomObjLogEntity> objList = doubleRandomLogList.Where(p => p.InspectorIds.Contains(userId) && p.FinishBy == "").ToList();
                        ...
                        var list = from left in doubleRandomLogList
                                   ...
                                   select new
                                   {
                                       ...
                                       FinishTime = string.IsNullOrEmpty(left.FinishTime) ? "" : DateTime.Parse(left.FinishTime).ToString("yyyy-MM-dd HH:mm:ss"),
                                       Remark = left.Remark,
                                       CreateOn = left.CreateOn.ToString("yyyy-MM-dd HH:mm:ss"),
                                       RandomType = new DoubleRandomSpotChecksBll().GetSearchList(new DoubleRandomSpotChecksEntity() { Id = left.ParentId })[0].RandomType,
                                       TypeName = new ComResourceBll().GetListBy("", left.TypeId)[0].RsKey,
                                       DeptName = new CrmDepartmentBll().GetAllDepartment(new CrmDepartmentEntity() { Id = left.TeamId })[0].ShortName
                                   };

                        Status = true;
                        obj = JsonConvert.SerializeObject(list).ToString();
                    }
                    else if (typeId == "1")
                    {
                        var list = from left in doubleRandomLogList
        ...
        /// <returns></returns>
        [WebMethod]
        public string ReportDoubleRandom(string context)
        {
            bool Status = false;
            var msg = "";
            object obj = null;
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(context))
                {
                    ...
                    entity.FinishBy = entity.InspectorIds;
                    entity.FinishPerson = entity.InspectorNames;
        ...
        /// <returns></returns>
        [WebMethod]
        public string GetImgList(string context)
        {
            bool Status = false;
            var msg = "";
            object obj = null;
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(context))
                {
                    ...
                    string Url = AppConfig.FileViewOutNetLink;

                    List<DoubleRandomAttachEntity> attList = new DoubleRandomAttachBll().Search(new DoubleRandomAttachEntity() { ResourceId = id });

                    var list = from left in attList
                               select new
                               {
                                   filePath = Url + left.FileAddress
                               };
                    Status = true;
                    obj = JsonConvert.SerializeObject(list).ToString();
                }
