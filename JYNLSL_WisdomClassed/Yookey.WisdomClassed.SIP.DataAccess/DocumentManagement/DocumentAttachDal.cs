...
{
    public class DocumentAttachDal : BaseDal<DocumentIncomingAttachEntity>
    {
        /// <summary>
        /// 数据查询
        /// </summary>
        ...
        public List<DocumentIncomingAttachEntity> GetAllResult(DocumentIncomingAttachEntity search)
        {
            ...
            {
                sbWhere.Append(Database.GetFormatSql<DocumentIncomingAttachEntity>(" and {0} = @p1 ", t => t.GL_RESOURCE_ID));
                args.Add(new { p1 = search.GL_RESOURCE_ID });
            }
            if (!string.IsNullOrEmpty(search.FILE_RENAME))
            {
                sbWhere.Append(Database.GetFormatSql<DocumentIncomingAttachEntity>(" and {0} like '" + search.FILE_RENAME + "%' ", t => t.FILE_RENAME));
            }
            if (!string.IsNullOrEmpty(search.ENCLOSURE_TYPE))
            {
                sbWhere.Append(Database.GetFormatSql<DocumentIncomingAttachEntity>(" and {0} = @p2 ", t => t.ENCLOSURE_TYPE));
                args.Add(new { p2 = search.ENCLOSURE_TYPE });
            }
            sbWhere.Append(" ORDER BY ENCLOSURE_TYPE ");
            return WriteDatabase.Query<DocumentIncomingAttachEntity>(sbWhere.ToString(), args.ToArray()).ToList();
        }
        ...
        {
            var strSql = string.Format(@"UPDATE DOCUMENT_INCOMING_ATTACH SET ROWSTATUS='{0}' WHERE ID='{1}'", (int)RowStatus.Delete, PK_ID);
        ...
        {
            var strSql = string.Format(@"INSERT INTO DOCUMENT_INCOMING_ATTACH (ID,GL_RESOURCE_ID,FILE_NAME,FILE_RENAME,FILE_STATE,ENCLOSURE_SIZE,FILE_ADDRESS,EVIDENCE_DATE,ENCLOSURE_TYPE,FILE_TYPE_NAME,REMARK,ROWSTATUS,CREATOR_ID,CREATE_BY,CREATE_ON,UPDATE_ID,UPDATE_BY,UPDATE_ON) 
