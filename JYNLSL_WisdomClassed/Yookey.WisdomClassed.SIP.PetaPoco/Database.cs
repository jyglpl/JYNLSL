...
namespace PetaPoco
{
    ...
    public class Database : IDisposable, IDatabase
    {
        ...
        public void OpenSharedConnection()
        {
            if (_sharedConnectionDepth == 0)
            {
                ...
                if (_sharedConnection == null)
                    _sharedConnection = _factory.CreateConnection();
                if (string.IsNullOrEmpty(_sharedConnection.ConnectionString))
                    _sharedConnection.ConnectionString = _connectionString;
        ...
        /// <returns>Page&lt;System.Object&gt;.</returns>
        /// <exception cref="System.Exception">Unable to parse SQL statement for paged query</exception>
        public Page<DataTable> Page(long page, long itemsPerPage, SQLParts parts, params object[] args)
        {
            string sqlCount, sqlPage;
            SQLParts outparts;
            if (!Provider.PagingUtility.SplitSQL(parts.Sql, out outparts))
                throw new Exception("Unable to parse SQL statement for paged query");
            outparts.SqlOrderBy = parts.SqlOrderBy;
            sqlPage = _provider.BuildPageQuery((page - 1) * itemsPerPage, itemsPerPage, outparts, ref args);
            sqlCount = outparts.SqlCount;

            int saveTimeout = OneTimeCommandTimeout;


            var result = new Page<DataTable>
            {
                CurrentPage = page,
                ItemsPerPage = itemsPerPage,
            ...
            result.Context = Query(sqlPage, args);
            return result;
        }
        ...
        /// <returns>Page&lt;System.Object&gt;.</returns>
        /// <exception cref="System.Exception">Unable to parse SQL statement for paged query</exception>
        public Page<T> Page<T>(long page, long itemsPerPage, SQLParts parts, params object[] args)
        {
            string sqlCount, sqlPage;
            SQLParts outparts;
            if (!Provider.PagingUtility.SplitSQL(parts.Sql, out outparts))
        ...
        public IEnumerable<T> QueryExtend<T>(string sql, params Expression<Func<T, object>>[] expressions)
        {
            ...
            sql = !rxFrom.IsMatch(sql) ? string.Format("SELECT {0} FROM {1} {2}", cols, GetTableName<T>(), sql) : string.Format("SELECT {0} {1}", cols, sql);
            OpenSharedConnection();
            try
            {
        ...
        /// <summary>
        /// 查询(返回DataTable，可用于多表查询) 周鹏 2016-02-23 添加
        /// </summary>
        ...
        /// <summary>
        /// 查询(返回DataSet，可用于多表查询) 周鹏 2016-02-23 添加
        /// </summary>
        /// <param name="sql">SQL Script</param>
        /// <param name="args">参数集</param>
        /// <returns></returns>
        ...
        {
            return Query<T>(sql, args).ToList().Single();
        }
        ...
        {
            return Query<T>(sql, args).ToList().SingleOrDefault();
        }
        ...
        {
            return Query<T>(sql, args).ToList().First();
        }
        ...
        {
            return Query<T>(sql, args).ToList().FirstOrDefault();
        }
        ...
        {
            return Query<T>(sql).ToList().Single();
        }
        ...
        {
            return Query<T>(sql).ToList().SingleOrDefault();
        }
        ...
        {
            return Query<T>(sql).ToList().First();
        }
        ...
        {
            return Query<T>(sql).ToList().FirstOrDefault();
        }
        ...
        public static string GetTableName<T>()
        {
            ...
            lock (lockobj)
            {
                tableName = ((TableNameAttribute)typeof(T).GetCustomAttributes(typeof(TableNameAttribute), false)[0]).Value;
        ...
        public StringExtension.SqlComment Comment { get; set; }
        ...
        /// <param name="args"></param>
        /// <returns></returns>
        public Page<DataTable> PageSql(long page, long itemsPerPage, SQLParts parts, params object[] args)
        {
            string sqlCount, sqlPage;
            SQLParts outparts;
            ...
            outparts.Sql = parts.Sql;
            PagingHelper pagingHelper = new PagingHelper();
            // Extract the columns from "SELECT <whatever> FROM"
            var m = pagingHelper.RegexColumns.Match(outparts.Sql);
            if (!m.Success)
                throw new Exception("Unable to parse SQL statement for paged query");

            // Save column list and replace with COUNT(*)
            var g = m.Groups[1];
            outparts.SqlSelectRemoved = parts.Sql.Substring(g.Index);

            if (pagingHelper.RegexDistinct.IsMatch(outparts.SqlSelectRemoved))
                outparts.SqlCount = parts.Sql.Substring(0, g.Index) + "COUNT(" + m.Groups[1].ToString().Trim() + ") " + parts.Sql.Substring(g.Index + g.Length);
            else
                outparts.SqlCount = parts.Sql.Substring(0, g.Index) + "COUNT(*) " + parts.Sql.Substring(g.Index + g.Length);

            #endregion

            outparts.SqlOrderBy = parts.SqlOrderBy;
            #region BuildPageQuery
            //sqlPage = _provider.BuildPageQuery((page - 1) * itemsPerPage, itemsPerPage, outparts, ref args);

            if (pagingHelper.RegexDistinct.IsMatch(outparts.SqlSelectRemoved))
            {
                outparts.SqlSelectRemoved = "peta_inner.* FROM (SELECT " + outparts.SqlSelectRemoved + ") peta_inner";
            }
            sqlPage = string.Format("SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>@{2} AND peta_rn<=@{3}",
                outparts.SqlOrderBy == null ? "ORDER BY (SELECT NULL)" : outparts.SqlOrderBy, outparts.SqlSelectRemoved, args.Length, args.Length + 1);
            args = args.Concat(new object[] { (page - 1) * itemsPerPage, (page - 1) * itemsPerPage + itemsPerPage }).ToArray();
    ...
    public static class StringExtension
    {
        ...
        private static string SqlAppendComment(this string sql,
             string author, string forUse, string fileName, string funName)
        {
            ...
            return
                ...
                --Flat:{5}/Author:{1}/For:{2}/File:{3}/Fun:{4}",
                    ...
                    fileName,
                    funName,
                    ConfigurationManager.AppSettings["ProjectName"] ?? string.Empty);
        }
             ...
             SqlComment comment)
        {
            ...
            {
                return sql.SqlAppendComment(comment.Author, comment.ForUse, comment.FileName, comment.FunName);
            }
            return sql;
        }
