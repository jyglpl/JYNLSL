    ...
    public class PictureServices
    {
        ...
        public string WritePicture(string fielType, string fileExt, byte[] ff)
        {
            lock (ob)
            {
                try
                {
                    string RootDirectory = AppConfig.FileSaveAddr;  //上传图片所对应的文件夹
                    string dirPath = RootDirectory + GetUpLoadPath(fielType);   //上传图片所对应的文件夹
                    ...
                    //执行写入图片
                    FileStream fs = new FileStream(RootDirectory + serverFileName, FileMode.CreateNew, FileAccess.Write, FileShare.None, ff.Length, false);
                ...
                {
                    string ErrorLog = AppConfig.FileSaveAddr;  //错误日志记录路径
                    string InsetFile = ErrorLog + @"\WritePicture" + DateTime.Now.ToLongDateString() + "Log.txt";
        ...
        {
            string UploadPath = AppConfig.FileSaveAddr;  //上传图片所对应的文件夹
            UploadPath += FileName;
        }
        ...
        public string MakewatermarkNew(string FilePath1, string FilePath2, string FilePath3, string FileName, string checktime, int width, int height, string mode, string type)
        {
            try
            {
                ...
                bool FExit1 = FilePath1 != "" ? System.IO.File.Exists(FileSave + FilePath1) : false;
                bool FExit2 = FilePath2 != "" ? System.IO.File.Exists(FileSave + FilePath2) : false;
                bool FExit3 = FilePath3 != "" ? System.IO.File.Exists(FileSave + FilePath3) : false;
                int towidth = width;
                int toheight = height;
                Bitmap bmp1 = null;
                Bitmap bmp2 = null;
        ...
        public string CopNewPic(string FilePath, string FileName)
        {
            try
            {
                ...
                System.IO.File.Copy(FileSave + FilePath, FileSave + serverFileName, true);
