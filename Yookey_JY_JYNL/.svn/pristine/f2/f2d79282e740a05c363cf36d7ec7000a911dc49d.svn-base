...
namespace Yookey.FTPService
{
    ...
    public class clsFTP
    {
        ...
        public bool DownloadFile(string RemoteFileName, string LocalPath, string dirName)
        {
            return DownloadFile(RemoteFileName, LocalPath, RemoteFileName, dirName);
        }
        /// <summary>
        ...
        public bool DownloadFile(string RemoteFileName, string LocalPath, string LocalFileName, string dirName)
        {
            byte[] bt = null;
            try
            {
                ...
                {
                    if (!Directory.Exists(LocalPath + dirName))
                    {
                        Directory.CreateDirectory(LocalPath + dirName);
                    }
                }

                string LocalFullPath = Path.Combine(LocalPath + dirName, LocalFileName);
        ...
        public bool UploadFile(byte[] FileBytes, string RemoteFileName, bool OverWriteRemoteFile)
        {
            ...
            {
                //i++;
                //if (ep.Message.ToString().IndexOf("The remote server returned an error: (500) Syntax error") >= 0 && i <= 2)
                //{
                //    if (!IsValidFileChars(RemoteFileName))
                //    {
                //        throw new Exception("非法文件名！");
