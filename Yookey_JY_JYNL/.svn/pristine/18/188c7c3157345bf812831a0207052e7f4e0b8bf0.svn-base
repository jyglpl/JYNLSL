...
namespace Yookey.WisdomClassed.SIP.Enumerate
{
    ...
    public class EnumOperate
    {
        ...
        /// <summary>
        /// 将指定枚举类型转换成Dictionary
        /// </summary>
        /// <param name="enumType">枚举类型</param>
        /// <returns></returns>
        public static Dictionary<string, string> ConvertEnumToDictionary(Type enumType)
        {
            if (enumType.IsEnum == false) { return new Dictionary<string, string>(); }
            var typeDescription = typeof(DescriptionAttribute);
            var fields = enumType.GetFields();
            ...
            foreach (var field in fields.Where(field => !field.IsSpecialName))
            {
                ...
                var arr = field.GetCustomAttributes(typeDescription, true);
                var strText = arr.Length > 0 ? ((DescriptionAttribute)arr[0]).Description : field.Name;
                dictionary.Add(strValue, strText);
            }
            return dictionary;
        }
