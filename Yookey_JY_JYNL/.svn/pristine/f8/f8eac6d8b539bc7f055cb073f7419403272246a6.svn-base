...
namespace Yookey.WisdomClassed.SIP.Common
{
    ...
    public static class Hz2Py
    {
        ...
        public static string ConvertToLower(string hzString)
        {
            return CommonMethod.FinalString(Convert(hzString)).ToLower();
        }
        ...
        /// 取单个字符的拼音声母
        /// </summary>
        /// <param name="cInput">要转换的单个汉字</param>
        /// <returns></returns>
        private static string GetPyChar(string cInput)
        {
            if (cInput == null) throw new ArgumentNullException("cInput");
            byte[] array = System.Text.Encoding.Default.GetBytes(cInput);
        ...
        /// <returns></returns>
        public static string GetPyPrefixForFirstHz(string str)
        {
            var s = string.Empty;
            if (!string.IsNullOrEmpty(str))
            {
                s = str.Length > 0 ? str.Substring(0, 1) : str;
            }
            return GetPyForShort(s);
        }
    }
    #endregion
}
