/**
 * jQuery LeaRunUI 4.1
 *苏州yookey Copyright © Learun 2014
 */
//样式
$(function () {
    readyIndex();
...
请求Ajax 带返回值
*/
function getAjaxUnAsync(url, postData, callBack) {
    //检测是否有虚拟路径
    url = urlFormat(url);
...
/*
 ...
 value: 传入文本值
 */
function SetOptionVal(id, value) {
    var selectValue = $(id).get(0);
    for (i = 0; i < selectValue.options.length; i++) {
        if (value == selectValue.options[i].text) {
...
type：类型 >1：成功，<1：失败，其他：警告
*/
function alertDialogCallBack(msg, type, callBack) {
    var msg = "<div class='ui_alert'>" + msg + "</div>";
    var icon = "";
    if (type >= 1) {
        icon = "succ.png";
...
function autocomplete(Objkey, width, height, data, callBack) {
    ...
    if (data != "") {
        ...
        sbhtml += '<tbody><tr><td style="color:red;text-align:center;width:' + width + ';"></td></tr></tbody>';
    }
    sbhtml += '</table>';
...
修改原因：程序中有的地方是完整路径，有的地方不是完整路径
*/
function BindDropItem(ControlId, ColName, Memo) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value='-999'>" + Memo + "</option>");
    }
    getAjax(RootPath() + '/PenaltyLaw/ComboxValue', { ColName: ColName }, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].Value).html(itemjson[i].Text));
        });
        Loading(false);
    });
}
...
修改原因：程序中有的地方是完整路径，有的地方不是完整路径
*/
function BindDropItem(ControlId, ColName, Memo, param, virtualPath) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value='-999'>" + Memo + "</option>");
    }
    getAjax(RootPath() + '/Punishment/ComboxValue', { ColName: ColName, param: param }, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].Value).html(itemjson[i].Text));
        });
...
function FieldExist(id, tablename, keyfield, Msg) {
    ...
    getAjax(RootPath() + '/Utility/FieldExist', postData, function (data) {
...
function SetDeriveExcel(obj, fileName) {
    ...
    getAjax(RootPath() + "/Utility/SetDeriveExcel", JsonData, function (data) { })
}
/*
表格视图列名
修改人：周庆
...
function columnModelData(jgrid) {
    AjaxJson(RootPath() + "/Utility/LoadViewColumn", {}, function (data) {
...
function PartialButton() {
    ...
    //AjaxJson(RootPath() + "/Utility/LoadButton", {}, function (Data) {
    //    JsonData = Data;
    //    $.each(Data, function (i) {
    ...
    //                toolbar_html += "<div style=\"float: left;\"><div class=\"icon\"><img src=\"" + RootPath() + "/Content/Images/Icon16/" + Data[i].Icon + "\" /></div><div class=\"text\">" + Data[i].FullName + "</div></div>";
    //                toolbar_html += "<div class=\"dropdown-icon\"><img src=\"" + RootPath() + "/Content/Images/dropdown-icon.png\" /></div>";
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace, IsVisitorModule) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','true')\"><a><img src='" + RootPath() + "/Content/Images/Icon16/" + img + "' width='16' height='16'>" + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");
        } else {
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class=\"selected\" onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','false')\"><a><img src='" + RootPath() + "/Content/Images/Icon16/" + img + "' width='16' height='16'>" + name + "</a></li>");
        }
...
/*
将完整或者不完整的url转换为完整的url
添加人：周庆
添加日期：2015年4月28日
添加原因：程序中的路径太杂乱了，统一转换
*/
